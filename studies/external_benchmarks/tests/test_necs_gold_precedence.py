...
def _frame(rows: list[dict]) -> pd.DataFrame:
    cols = ["chemical_name", "gold_inchikey", "gold_smiles",
            "gold_inchikey_standard", "gold_smiles_standard", "gold_formula"]
...
def test_kind_a_row_resolves_offline_to_self_consistent_vintage():
    ...
    df = _frame([dict(chemical_name="cortisone",
...
def test_agreeing_row_passes_through_to_standard_form():
    ...
    df = _frame([dict(chemical_name="ethanol", gold_inchikey=key_l, gold_smiles=smi,
                      gold_inchikey_standard=key_l, gold_smiles_standard=smi, gold_formula="C2H6O")])
    out = build_repaired_gold(df)
...
def test_kind_b_row_is_pending_anchor_never_defaulted():
    df = _frame([dict(chemical_name="choline", gold_inchikey=_CHOLINE_L_KEY, gold_smiles=_CHOLINE_L_SMI,
                      gold_inchikey_standard=_CHOLINE_M_KEY, gold_smiles_standard=_CHOLINE_M_SMI,
...
def test_single_vintage_and_no_gold_rows():
    df = _frame([
        ...
        dict(chemical_name="modern_only", gold_inchikey_standard="CCCCCCCCCCCCCC-DDDDDDDDDD-E",
...
def test_repair_is_total_states_partition_the_frame():
    df = _frame([
        dict(chemical_name="a", gold_inchikey=sc.standard_inchikey("CCO"), gold_smiles="CCO",
             gold_inchikey_standard=sc.standard_inchikey("CCO"), gold_smiles_standard="CCO"),
        dict(chemical_name="choline", gold_inchikey=_CHOLINE_L_KEY, gold_smiles=_CHOLINE_L_SMI,
             gold_inchikey_standard=_CHOLINE_M_KEY, gold_smiles_standard=_CHOLINE_M_SMI),
...
def test_positive_control_bad_modern_key_flips_precedence():
    ...
    df = _frame([dict(chemical_name="rigged",
                      gold_inchikey=sc.standard_inchikey("CCO"), gold_smiles="CCO",
                      gold_inchikey_standard="WRONGKEYXXXXXX-ZZZZZZZZZZ", gold_smiles_standard="CCC",
                      gold_formula="")])
    out = build_repaired_gold(df)
    assert out["repair_state"].iloc[0] == "resolved_offline"
