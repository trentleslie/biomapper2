"""KestrelVectorSearchAnnotator: scan the ranked window for the first on-category node.
...
def _annotator(monkeypatch, rows_by_term):
    ann = KestrelVectorSearchAnnotator()
    captured = {}
    ...
    monkeypatch.setattr(ann, "_kestrel_vector_search", _fake)
    return ann, captured
